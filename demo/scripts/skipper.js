...
(function() {
  var Ajax, InvalidRequestAPI, InvalidRequestData, InvalidRequestParam, InvalidRequestType, OPEN_ID_COOKIE, PARAM_WX_OPEN_SID, PROFILE_COOKIE, Q, ResouceNotFound, TOKEN_COOKIE, WX_LINK_COOKIE, WX_OPEN_SID_COOKIE, is_exports, procces_cookie_input, procces_cookie_output, root, supCookie, utils, version;
  ...
  root.Skipper = function(opts) {
    ...
    load_opt = function(options, key) {
      var html, j, len, meta, metas, opt;
      opt = null;
      if (options[key]) {
        opt = options[key];
      }
      if (!opt) {
        ...
        opt = html.getAttribute(key) || html.dataset[key];
      }
      if (!app_id) {
    ...
    member = {
      ...
      parse_form: function(form_element) {
        var _get_field, action, data, data_fields, elem, elems, error2, invalid_fields, j, l, len, len1, len2, m, msg, msgs, status;
        try {
        ...
        for (j = 0, len = elems.length; j < len; j++) {
          ...
          for (l = 0, len1 = msgs.length; l < len1; l++) {
            msg = msgs[l];
      ...
      mailto: function(action, data_fields) {
        var field, j, len, mail_content, mail_data, ref, subject;
      ...
      demand: {
        free: function(event_slug, data_fields, success, failed) {
          var field, fields, j, len, ref, subject;
          subject = '';
          fields = [];
          ref = data_fields || [];
          for (j = 0, len = ref.length; j < len; j++) {
